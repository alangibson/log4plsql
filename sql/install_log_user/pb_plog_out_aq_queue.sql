...
BEGIN
   ...
   -- Make sure queue name is always absolute
   l_queue_name := SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') || '.LOG_QUEUE';
...
BEGIN        
    ...
    IF get_queue_msg_count > 0 THEN
      ...
      BEGIN
        -- Make sure queue name is absolute 
        l_queue_name := SYS_CONTEXT('USERENV', 'CURRENT_SCHEMA') || '.LOG_QUEUE';
...
--******************************************************************************
  ...
  IF l_nr_msg_queue >  0 THEN
    ...
    BEGIN
      ...
      l_cnt :=  DBMS_AQ.DEQUEUE_ARRAY(queue_name    => l_queue_name,
                          dequeue_options           => l_dequeue_options,
